function GetTableLumberjackScript_v3()
...
local function Attack()
	...
	then 
		...
		lua_table.SystemFunctions:LOG("DELAY BEFORE ATTACK")
...
local function HandleSEEK()
	...
	then
		...
		then
			lua_table.SystemFunctions:LOG("STATE ALERT")
	...
	then
		...
		then
			...
			lua_table.SystemFunctions:LOG("SEEK TARGET ANIM RUN")
	...
	then
		...
		lua_table.SystemFunctions:LOG("Between 14 and 15 ---- JumpAttack()")
	end
...
function lua_table:RequestedTrigger(collider_GO)
	lua_table.SystemFunctions:LOG("On RequestedTrigger special effect requested")
	...
	then
		...
		then
			...
			then
				lua_table.SystemFunctions:LOG("requested a stun")
			...
			then
				lua_table.SystemFunctions:LOG("requested a knockback")
