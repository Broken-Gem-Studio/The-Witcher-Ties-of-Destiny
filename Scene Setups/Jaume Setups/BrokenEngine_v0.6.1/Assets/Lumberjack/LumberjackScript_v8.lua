function GetTableLumberjackScript_v8()
...
--_JUMPATTACK()--NEW
local _AuxTargetSelected = false
local _AuxTarget = {}
--Die()
...
local function NormalizeDirVector_AuxTarget()
	...
	Nvec3x = vec3x / _AuxTargetDistanceMagnitude 
	Nvec3z = vec3z / _AuxTargetDistanceMagnitude -- Normalized values for jumpattack
	VectorNormalized = true
...
local function _JumpAttack()

	if _AuxTargetSelected == false
	then
		_AuxTarget = lua_table.TransformFunctions:GetPosition(lua_table.CurrentTarget) 
		_AuxTargetSelected = true
	end

	if _AuxTargetSelected == true
	then
		_AuxTargetDistanceMagnitude = CalculateDistanceToPosition(_AuxTarget)
	end

	if _AuxTargetDistanceMagnitude >= 7 --preparing jump
	then
		_JumpAttackSpeed = 2
	elseif _AuxTargetDistanceMagnitude < 7 --jumping
	then
		_JumpAttackSpeed = 4
	elseif _AuxTargetDistanceMagnitude < MinDistanceFromPlayer --arrived to target
	then
		_JumpAttackSpeed = 0
	end
...
local function HandleSEEK()
	...
	--#####################################################################################   SEEK DONE
	if DistanceMagnitude < 10 and DistanceMagnitude > 9 and _JumpAttackDone == false--
	then
		_DoJumpAttack = true--
	end

	if _DoJumpAttack == true
	then
		if _CancelateJumpAttack == false
		then
			_JumpAttack()
		end
	end
