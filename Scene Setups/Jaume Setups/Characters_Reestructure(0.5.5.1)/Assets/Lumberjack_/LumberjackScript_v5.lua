function GetTableLumberjackScript_v5()
...
local ChangeDirectionToNegative = false
local ChangeDirectionToPositive = false
...
local function ApplyVelocity()
	...
	then 
		lua_table.SystemFunctions:LOG("LUMBERJACK WTF ######################")
	...
	then
		lua_table.SystemFunctions:LOG("LUMBERJACK WTF 2 ######################")
		if DistanceMagnitude > lua_table.MinDistanceFromPlayer and lua_table.CurrentSubState == SubState.SEEK_TARGET 
		then
			lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3x MAX SPEED = "..Nvec3x)
			lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3y MAX SPEED = "..Nvec3z)
	...
	then
		Nvec3x = 0
		Nvec3z = 0
		lua_table.SystemFunctions:LOG("LUMBERJACK WTF 3 ######################")
	end
	
	if Do_KnockBack == true and lua_table.CurrentSpecialEffect == SpecialEffect.KNOCKBACK
	then
		lua_table.SystemFunctions:LOG("LUMBERJACK Aplying KnockBackVelocity")
		--lua_table.SystemFunctions:LOG("LUMBERJACK distanceMagnitude = "..DistanceMagnitude)
		lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3x = "..Nvec3x)
		lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3z = "..Nvec3z)
		lua_table.SystemFunctions:LOG("LUMBERJACK KnockbackVelocity = "..KnockbackVelocity)
...
local function HandleAttack()
	...
	then 
		...
		lua_table.CurrentSubState = SubState.SEEK_TARGET
		lua_table.JumpAttackDone = true
		--lua_table.SystemFunctions:LOG("LUMBERJACK ATTACK----->SEEK")
...
function lua_table:Start()
	...
	--ua_table.ParticleSystem:StopParticleEmitter(particles.alert1.GO_UID)
	...
	CurrTime = PerfGameTime()
		...
		then
			...
			then
				...
				if CurrTime - KnockbackTimeController > 6000
				then
					lua_table.CurrentSpecialEffect = SpecialEffect.NONE
					knockback_AnimController = false
					StandUpAnimController = false
					ChangeDirectionToPositive = false
					ChangeDirectionToNegative = false
					lua_table.SystemFunctions:LOG("LUMBERJACK HAN PASADO 6 SEGUNDOS YA @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
				end
		...
		then
			...
			lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3x before moving = "..Nvec3x)
			lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3y before moving = "..Nvec3z)
			lua_table.SystemFunctions:LOG("LUMBERJACK Nvec3y before moving lua_table.CurrentSubState = "..lua_table.CurrentSubState)
