function GetTableCameraScript_v9 ()
local lua_table = {}
...
local current_light_distance = lua_table.light_distance -- Should initialize at awake(?)
local current_light_angle = lua_table.light_angle -- Should initialize at awake(?)
local current_light_angle_for_offset = 0 -- 90-lua_table.current_light_angle
local current_light_orientation = lua_table.light_orientation

local camera_orientation_rad = 0
local camera_orientation_sin = 0
local camera_orientation_cos = 0

local light_orientation_rad = 0
local light_orientation_sin = 0
local light_orientation_cos = 0
...
-- Light position
local light_position_x = 0
local light_position_y = 0 
local light_position_z = 0
...
-- Light Desired distance
local light_desired_position_x = 0
local light_desired_position_y = 0
local light_desired_position_z = 0
...
-- Light Distance Offset
local light_offset_a = 0
local light_offset_b = 0
...
-- Light rotation (z hardcoded for now bc reasons)
local light_rotation_x = 0 
local light_rotation_y = 0 
local light_rotation_z = 0
...
local function HandleTarget()
	...
	then
		...
		then
			...
			then
				...
				target_position_x = Centroid2P(lua_table.P1_pos[x], lua_table.Kikimora_pos[x])
				target_position_y = Centroid2P(lua_table.P1_pos[y], lua_table.Kikimora_pos[y])
				target_position_z = Centroid2P(lua_table.P1_pos[z], lua_table.Kikimora_pos[z])
			end	
		...
		then
			...
			then
				...
				target_position_x = lua_table.P2_pos[x]
				target_position_y = lua_table.P2_pos[y]		-- Kind of redundant but organized
				target_position_z = lua_table.P2_pos[z]
			else
				-- Target is Midpoint between P1 and BOSS positions
				target_position_x = Centroid2P(lua_table.P2_pos[x], lua_table.Kikimora_pos[x])
				target_position_y = Centroid2P(lua_table.P2_pos[y], lua_table.Kikimora_pos[y])
				target_position_z = Centroid2P(lua_table.P2_pos[z], lua_table.Kikimora_pos[z])
			end	
		end
	...
	then
		...
		then
			...
			target_position_x = Centroid3P(lua_table.P1_pos[x], lua_table.P2_pos[x], lua_table.Kikimora_pos[x])
			target_position_y = Centroid3P(lua_table.P1_pos[y], lua_table.P2_pos[y], lua_table.Kikimora_pos[y])
			target_position_z = Centroid3P(lua_table.P1_pos[z], lua_table.P2_pos[z], lua_table.Kikimora_pos[z])
		end
...
local function HandleOffset()
	...
	current_light_angle_for_offset = 90 - current_light_angle

	current_light_orientation = 0 - lua_table.current_camera_orientation
	...
	-- Light Offset from Distance and Angle
	light_offset_a = GetAfromDistAndAng(current_light_distance, current_light_angle_for_offset)
	light_offset_b = GetBfromDistAndAng(current_light_distance, current_light_angle_for_offset)
	...
	-- Offset Calculations for Light
    light_orientation_rad = math.rad(current_light_orientation)
    light_orientation_sin = math.sin(light_orientation_rad)
    light_orientation_cos = math.cos(light_orientation_rad)
	
	light_offset_x = light_offset_b * light_orientation_sin - camera_orientation_sin
	light_offset_y = light_offset_a  
	light_offset_z = light_offset_b * light_orientation_cos - camera_orientation_cos
	...
	light_rotation_x = current_light_angle - 180 -- I really don't know why I "have" to do this
	...
	-- Rotation of the Light (orientation)
	lua_table.TransformFunctions:SetObjectRotation(light_rotation_x, current_light_orientation, light_rotation_z, lua_table.light_UID)
...
local function HandleMovement()
	...
	then
		...
		then
			...
			camera_position_x = target_position_x + camera_offset_x
			camera_position_y = target_position_y + camera_offset_y 	-- Kind of redundant but conceptually organized
			camera_position_z = target_position_z + camera_offset_z

			light_position_x = target_position_x + light_offset_x
			light_position_y = target_position_y + light_offset_y
			light_position_z = target_position_z + light_offset_z
		...
		then
			...
			camera_desired_position_x = target_position_x + camera_offset_x
			camera_desired_position_y = target_position_y + camera_offset_y
			camera_desired_position_z = target_position_z + camera_offset_z

			-- Light Desired Position
			light_desired_position_x = target_position_x + light_offset_x 
			light_desired_position_y = target_position_y + light_offset_y
			light_desired_position_z = target_position_z + light_offset_z
			...
			light_position_x = Asymptotic_Average(light_position_x, light_desired_position_x, lua_table.movement_smooth_speed)
			light_position_y = Asymptotic_Average(light_position_y, light_desired_position_y, lua_table.movement_smooth_speed)
			light_position_z = Asymptotic_Average(light_position_z, light_desired_position_z, lua_table.movement_smooth_speed)
		end

		-- Setting Camera Position
		lua_table.TransformFunctions:SetPosition(camera_position_x, camera_position_y, camera_position_z, lua_table.my_UID)

		-- Setting Light Position(NEEDS FIX, NOW IS HARDCODED FOR REASONS)
		lua_table.TransformFunctions:SetPosition(light_position_x, light_position_y, light_position_z, lua_table.light_UID)
	end
end
...
function lua_table:Awake ()
    ...
    -- Get Light UID
    lua_table.light_UID = lua_table.GameObjectFunctions:FindGameObject(lua_table.light_GO)

    if lua_table.light_UID == 0
    then
        lua_table.SystemFunctions:LOG ("Camera: can't find light")
    end
	...
	current_light_angle_for_offset = 90 - lua_table.light_angle
	light_rotation_x = 180 - lua_table.light_angle 
	-- Debug
