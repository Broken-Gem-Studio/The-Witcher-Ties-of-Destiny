...
lua_table.medium_2_collider_front_start = 250	--Collider activation time
lua_table.medium_2_collider_front_end = 400	--Collider deactivation time
...
local function CheckCameraBounds()	--Check if we're currently outside the camera's bounds
	...
	local camera_Y_rot = math.rad(GimbalLockWorkaroundY(camera_test_GO_UID))
	
	--3. Generate a vector and change angle depending on planes that we're traspassing (1 plane = 90º, 2 planes = 45º)
	--3.1. Check down/up
	if sides[2] == 0 then
		bounds_vector.z = -math.cos(camera_Y_rot)
	...
	if sides[3] == 0 then
		bounds_vector.x = math.sin(camera_Y_rot)
...
function lua_table:Awake()
	...
	camera_test_GO_UID = lua_table.GameObjectFunctions:FindGameObject("Camera_Test_Parent")
