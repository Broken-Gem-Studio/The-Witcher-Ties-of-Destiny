...
function	GetTableCameraScript_v7 ()
local lua_table = {}
...
lua_table.camera_bossfight_distance = 40
...
lua_table.camera_bossfight_angle = 50
...
local rotation_x = 0 -- 180 - (current_camera_angle) 
...
-- Level stage
local stage = -- not in use rn
{
	LEVEL_1 = 1, 
	LEVEL_2 = 2,
}
local current_stage = stage.LEVEL_1 -- THIS IS SO HARCODED 
local bossfight = false -- Will be true when Kikimora gets inside frustum
...
local current_gameplay = 0 --AUTOMATICALLY initializes at awake (not hardcoded anymore)
...
-- 		then
-- 			bossfight = true
-- 			lua_table.SystemFunctions:LOG ("Camera: BOSSFIGHT")
-- 		end
-- 	end
-- end
...
local function HandleZoomLayers()
	...
	then
        ...
        then
			...
			if bossfight == true
            then
                if current_zoom_layer ~= zoom.LAYER_BOSS
                then 
                    -- Switch up to Layer Boss
					current_zoom_layer = zoom.LAYER_BOSS
					current_state = state.SWITCHING
...
local function HandleSwitch()
	...
	then
        ...
        elseif current_zoom_layer == zoom.LAYER_BOSS
        then
            desired_distance = lua_table.camera_bossfight_distance
            current_camera_distance = Asymptotic_Average(current_camera_distance, desired_distance, lua_table.zoom_distance_smooth_speed)
            
            desired_angle = lua_table.camera_bossfight_angle
            current_camera_angle = Asymptotic_Average(current_camera_angle, desired_angle, lua_table.zoom_angle_smooth_speed) --Smoothens angle transition
...
local function HandleTarget()
    ...
    then
        ...
        then
            ...
            if P1_dead == true
            then
                if lua_table.P1_script.current_state ~= -4 --NOT DEAD
                then
            ...
            elseif P2_dead == true
            then
                if lua_table.P2_script.current_state ~= -4 --NOT DEAD
                then
                    current_gameplay = gameplay.DUO
                end
            end
...
local function HandleOffset()
	...
	offset_a = GetAfromDistAndAng(current_camera_distance, current_camera_angle_for_offset)
	offset_b = GetBfromDistAndAng(current_camera_distance, current_camera_angle_for_offset)
    ...
    -- to support changes in the angle we must update the rotation_x aswell so it keeps the orientation to the target
    rotation_x = 180 - current_camera_angle
	lua_table.TransformFunctions:SetObjectRotation(rotation_x, rotation_y, rotation_z, lua_table.myUID)
end
...
function lua_table:Awake ()
	...
	rotation_x = 180 - lua_table.camera_angle_layer_1 --because soemthing is wrong with the motor view that is inverted
