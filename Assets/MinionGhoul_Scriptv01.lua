function GetTableMinionGhoul_Scriptv011()
local lua_table = {}
...
local Minion_Attack_Collider = 0
...
local function Attack()
	...
	if attack_timer + 250 <= lua_table.System:GameTime() * 1000 and not punching then
		lua_table.System:LOG("Punch to target")
		lua_table.Animations:PlayAnimation("Punch", 30.0, lua_table.MyUID)
		punching = true
	end
	
	-- if attack_timer + 1000 <= lua_table.System:GameTime() * 1000 and attack_timer + 1100 >= lua_table.System:GameTime() * 1000 then  ------------------------------ UNCOMMENT WHEN ADDING COLLIDERS
	...
	if attack_timer + 1300 <= lua_table.System:GameTime() * 1000 then
		lua_table.currentState = State.SEEK	
		lua_table.Animations:PlayAnimation("Walk", 40.0, lua_table.MyUID)
...
function lua_table:Update()
	...
	elseif lua_table.currentState == State.SEEK then -- and lua_table.is_stunned == false 
		Seek()
	elseif lua_table.currentState == State.ATTACK then    	-- and not lua_table.is_stunned == false
		Attack()
