...
function HandlePhases()
    ...
    if  current_health_percentage <= lua_table.health_percentage_phase_2 and current_health_percentage >= lua_table.health_perecntage_phase_3
    then
    ...
    elseif current_health_percentage <= lua_table.health_perecntage_phase_3 and current_health_percentage >= lua_table.health_perecntage_phase_4
    then
    ...
    elseif current_health_percentage <= lua_table.health_perecntage_phase_4
    then
        if current_phase ~= phase.MAD
        then
            current_state = state.SWAPPING_PHASE
...
function HandleRoarAttack()
    ...
    then
        ...
        lua_table.AnimationFunctions:PlayAnimation(animation.roar.anim_name, animation.roar.anim_speed)
        attack_timer = game_time + attack.roar.att_duration
        lua_table.SystemFunctions:LOG ("Kikimora: Roar Start")

    elseif current_attack_type == attack_type.ATTACKING_ROAR --Checks if is started
    then
        if game_time <= attack_timer --Checks if ongoing
        then
            -- Activates collider                               TODO: TIME WELL COLLIDERS
            lua_table.GameObjectFunctions:SetActiveGameObject(true, attack_colliders.roar.GO_name)
            -- Sets damage
            lua_table.collider_damage = attack.roar.att_damage
            -- Sets effect
            lua_table.collider_effect = attack_effects.none -- attack_effects.knockback TODO: Figure out how to send knockback to players.
        ...
        then
            ...
            lua_table.GameObjectFunctions:SetActiveGameObject(false, attack_colliders.roar.GO_name)
...
function HandleStompAttack()
    ...
    then
        ...
        lua_table.AnimationFunctions:PlayAnimation(animation.stomp.anim_name, animation.stomp.anim_speed)
        attack_timer = game_time + attack.stomp.att_duration
        lua_table.SystemFunctions:LOG ("Kikimora: Stomp Start")

    elseif current_attack_type == attack_type.ATTACKING_STOMP --Checks if is started
    then
        if game_time <= attack_timer --Checks if ongoing
        then
            -- Activates collider                               TODO: TIME WELL COLLIDERS
            lua_table.GameObjectFunctions:SetActiveGameObject(true, attack_colliders.stomp.GO_name)
            -- Sets damage
        ...
        then
            ...
            lua_table.GameObjectFunctions:SetActiveGameObject(false, attack_colliders.stomp.GO_name)
            current_attack_type = attack_type.TO_BE_DETERMINED
...
function HandleSweepAttack()
    ...
    then
        ...
        lua_table.AnimationFunctions:PlayAnimation(animation.sweep.anim_name, animation.sweep.anim_speed)
        attack_timer = game_time + attack.sweep.att_duration
        lua_table.SystemFunctions:LOG ("Kikimora: Sweep Start")

    elseif current_attack_type == attack_type.ATTACKING_SWEEP --Checks if is started
    then
        if game_time <= attack_timer --Checks if ongoing
        then
            -- Activates collider                               TODO: TIME WELL COLLIDERS
            lua_table.GameObjectFunctions:SetActiveGameObject(true, attack_colliders.sweep.GO_name)
            -- Sets damage
        ...
        then
            ...
            lua_table.GameObjectFunctions:SetActiveGameObject(false, attack_colliders.sweep.GO_name)
            current_attack_type = attack_type.TO_BE_DETERMINED
...
function HandleLeashAttack()
    ...
    then
        ...
        lua_table.AnimationFunctions:PlayAnimation(animation.leash.anim_name, animation.leash.anim_speed)
        attack_timer = game_time + attack.leash.att_duration
        lua_table.SystemFunctions:LOG ("Kikimora: Leash Start")

    elseif current_attack_type == attack_type.ATTACKING_LEASH --Checks if is started
    then
        if game_time <= attack_timer --Checks if ongoing
        then
            -- Activates collider                               TODO: TIME WELL COLLIDERS
            lua_table.GameObjectFunctions:SetActiveGameObject(true, attack_colliders.leash.GO_name)
            -- Sets damage
        ...
        then
            ...
            lua_table.GameObjectFunctions:SetActiveGameObject(false, attack_colliders.leash.GO_name)
            current_attack_type = attack_type.TO_BE_DETERMINED
...
function HandleAttacks()
    ...
    if current_state ~= state.ATTACKING and current_state ~= state.AWAKENING and current_state ~= state.SWAPPING_PHASE
    then
...
function HandleStates()
    ...
    then
        if game_time >= state_timer
        then
            lua_table.AnimationFunctions:PlayAnimation(animation.idle.anim_name, animation.idle.anim_speed)
    ...
    then
        ...
        then
            ...
            lua_table.AnimationFunctions:PlayAnimation(animation.roar.anim_name, animation.roar.anim_speed / 2) -- It will have a times 2 duration
